...
<div class="max-w-7xl mx-auto space-y-8">
    ...
    <div class="bg-white border border-slate-200/90 rounded-3xl p-8 space-y-6 shadow-sm">
        <div>
            <h1 class="text-2xl font-extrabold text-slate-950 tracking-tight font-sans">Create Promotional Code</h1>
            <p class="text-slate-500 text-xs font-medium mt-1">Define promotional coupon discounts to share with hosts or clients.</p>
        </div>

        <form action="<?= APP_URL ?>/admin/promo-codes" method="POST" class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div>
                    ...
                    <input type="text" name="code" required placeholder="WELCOME20"
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="discount_value" required placeholder="20.00"
    ...
    <div class="bg-white border border-slate-200/90 rounded-3xl p-8 space-y-6 shadow-sm">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-xl font-extrabold text-slate-950 tracking-tight font-sans">All Promotional Codes</h2>
                <p class="text-slate-500 text-xs font-medium mt-1">List of active coupon offers and discount rates.</p>
            </div>
            <span class="text-xs text-slate-500 font-bold bg-slate-100 px-3 py-1.5 rounded-xl border border-slate-200"><?= count($promoCodes) ?> Codes</span>
        </div>
                    ...
                    <?php else: ?>
                        <?php foreach ($promoCodes as $pc): ?>
                            <tr class="hover:bg-slate-50/60 transition-colors">
                                ...
                                <td class="px-6 py-4 text-xs font-bold text-indigo-700">
                                ...
                                <td class="px-6 py-4 flex items-center justify-end">
                                    <form id="delete-promo-form-<?= $pc['id'] ?>" method="POST" action="<?= APP_URL ?>/admin/promo-codes/delete/<?= $pc['id'] ?>" class="inline-block">
                                        <input type="hidden" name="csrf_token" value="<?= $csrf_token ?>">
...
<script>
    function confirmDeletePromo(promoId) {
        Swal.fire({
            ...
            text: "Are you sure you want to delete this promo code? It will immediately stop working for any new bookings.",
            icon: 'warning',
